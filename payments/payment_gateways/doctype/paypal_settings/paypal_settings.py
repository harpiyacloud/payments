...
from harpiya.integrations.utils import create_request_log, make_post_request
from harpiya.model.document import Document
from harpiya.utils import call_hook_method, cint, get_datetime, get_url
...
class PayPalSettings(Document):
	...
	def setup_sandbox_env(self, token):
		data = json.loads(harpiya.db.get_value("Integration Request", token, "data"))
		setattr(self, "use_sandbox", cint(harpiya._dict(data).use_sandbox) or 0)
	...
	def get_paypal_params_and_url(self):
		...
		if hasattr(self, "use_sandbox") and self.use_sandbox:
			params.update(
				{
					"USER": harpiya.conf.sandbox_api_username,
					"PWD": harpiya.conf.sandbox_api_password,
					"SIGNATURE": harpiya.conf.sandbox_signature,
				}
			)
	...
	def validate_paypal_credentails(self):
		...
		try:
			...
			harpiya.throw(_("Invalid payment gateway credentials"))
...
def get_paypal_and_transaction_details(token):
	doc = harpiya.get_doc("PayPal Settings")
	doc.setup_sandbox_env(token)
	params, url = doc.get_paypal_params_and_url()

	integration_request = harpiya.get_doc("Integration Request", token)
...
def setup_redirect(data, redirect_url, custom_redirect_to=None, redirect=True):
	...
	if redirect:
		harpiya.local.response["type"] = "redirect"
		harpiya.local.response["location"] = get_url(redirect_url)


@harpiya.whitelist(allow_guest=True, xss_safe=True)
	...
	try:
		...
		if response.get("ACK")[0] != "Success":
			harpiya.respond_as_web_page(
				...
				indicator_color="red",
				http_status_code=harpiya.ValidationError.http_status_code,
			)

			return

		doc = harpiya.get_doc("Integration Request", token)
		...
		harpiya.local.response["type"] = "redirect"
		harpiya.local.response["location"] = get_redirect_uri(
			doc, token, response.get("PAYERID")[0]
		)

	except Exception:
		harpiya.log_error(harpiya.get_traceback())
	...
	try:
		...
		if response.get("ACK")[0] == "Success":
			...
			if data.get("reference_doctype") and data.get("reference_docname"):
				custom_redirect_to = harpiya.get_doc(
...
def create_recurring_profile(token, payerid):
	try:
		...
		starts_at = (
			get_datetime(subscription_details.get("start_date")) or harpiya.utils.now_datetime()
		)
		starts_at = starts_at.replace(
			tzinfo=pytz.timezone(harpiya.utils.get_time_zone())
		).astimezone(pytz.utc)
...
def manage_recurring_payment_profile_status(profile_id, action, args, url):
	...
	if (
		...
		harpiya.throw(_("Failed while amending subscription"))


@harpiya.whitelist(allow_guest=True)
def ipn_handler():
	try:
		data = harpiya.local.form_dict
		...
		doc = harpiya.get_doc(
			{
				"data": json.dumps(harpiya.local.form_dict),
	...
	except harpiya.InvalidStatusError:
		pass
	except Exception as e:
		harpiya.log(harpiya.log_error(title=e))


def validate_ipn_request(data):
	def _throw():
		harpiya.throw(_("In Valid Request"), exc=harpiya.InvalidStatusError)
