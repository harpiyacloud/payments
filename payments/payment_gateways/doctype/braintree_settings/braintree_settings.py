...
from harpiya.integrations.utils import create_request_log
from harpiya.model.document import Document
from harpiya.utils import call_hook_method, get_url
...
class BraintreeSettings(Document):
	...
	def create_payment_request(self, data):
		...
		try:
			...
			return {
				"redirect_to": harpiya.redirect_to_message(
					_("Server Error"),
					_(
		...
		if self.flags.status_changed_to == "Completed":
			...
			if self.data.reference_doctype and self.data.reference_docname:
				...
				try:
					...
					braintree_success_page = harpiya.get_hooks("braintree_success_page")
					if braintree_success_page:
						custom_redirect_to = harpiya.get_attr(braintree_success_page[-1])(self.data)
				except Exception:
					harpiya.log_error(harpiya.get_traceback())
...
def get_gateway_controller(doc):
	payment_request = harpiya.get_doc("Payment Request", doc)
	gateway_controller = harpiya.db.get_value(
...
def get_client_token(doc):
	...
	settings = harpiya.get_doc("Braintree Settings", gateway_controller)
