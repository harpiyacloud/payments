...
from harpiya.custom.doctype.custom_field.custom_field import create_custom_fields


def get_payment_gateway_controller(payment_gateway):
	"""Return payment gateway controller"""
	gateway = harpiya.get_doc("Payment Gateway", payment_gateway)
	if gateway.gateway_controller is None:
		try:
			return harpiya.get_doc(f"{payment_gateway} Settings")
		except Exception:
			harpiya.throw(_("{0} Settings not found").format(payment_gateway))
	else:
		try:
			return harpiya.get_doc(gateway.gateway_settings, gateway.gateway_controller)
		except Exception:
			harpiya.throw(_("{0} Settings not found").format(payment_gateway))
...
def get_checkout_url(**kwargs):
	try:
		if kwargs.get("payment_gateway"):
			doc = harpiya.get_doc("{} Settings".format(kwargs.get("payment_gateway")))
...
def create_payment_gateway(gateway, settings=None, controller=None):
	...
	if not harpiya.db.exists("Payment Gateway", gateway):
		payment_gateway = harpiya.get_doc(
			{
...
def make_custom_fields():
	if not harpiya.get_meta("Web Form").has_field("payments_tab"):
		...
		harpiya.clear_cache(doctype="Web Form")


def delete_custom_fields():
	if harpiya.get_meta("Web Form").has_field("payments_tab"):
		...
		for fieldname in fieldnames:
			harpiya.db.delete("Custom Field", {"name": "Web Form-" + fieldname})
...
def before_install():
	# TODO: remove this
	# This is done for harpiyaerp CI patch test
	#
	...
	# a lot of apis don;t exist in v10 and this is a (at the moment) required app for harpiyaerp.
	if not harpiya.get_meta("Module Def").has_field("custom"):
		return False
